...
import { Calendar, Clock, Brain, FileText, ChevronRight, ChevronDown, ChevronUp, Loader2, Search } from "lucide-react"
...
export function CreateWorkflowModal({ open, onClose, onWorkflowCreated, sites }: CreateWorkflowModalProps) {
  ...
  const [isDescGenerating, setIsDescGenerating] = useState<Record<string, boolean>>({})
  ...
  // Plan state: plan[weekNum][dayName] = { title, keywords, imageFile, baseTopic }
  ...
  useEffect(() => {
    if (open) {
      resetForm()
      initPlan(1, DAYS) // Reset to 1 week, Daily days
    }
  }, [open])
  ...
  async function generateDescriptionForDay(week: number, day: string, dayData: { title: string; keywords: string }) {
    const key = `${week}-${day}`
    if (!dayData.title.trim()) {
      alert("Please enter a post title before generating a description")
      return
    }

    setIsDescGenerating(prev => ({ ...prev, [key]: true }))
    updateDay(week, day, "description", "")
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/posts/generate`, {
        ...
        body: JSON.stringify({
          topic: dayData.title,
          keywords: dayData.keywords.split(",").map(k => k.trim()).filter(Boolean),
          tone,
          wordCount,
          language,
        }),
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (data.content) {
              ...
              updateDay(week, day, "description", generatedHtml)
            }
          } catch {
      ...
      setIsDescGenerating(prev => ({ ...prev, [key]: false }))
    }
  }
  ...
  async function handleSubmit() {
    ...
    if (scheduleType !== "cron") {
      for (let w = 1; w <= numWeeks; w++) {
        for (const day of activeDays) {
          const data = plan[`week${w}`]?.[day]
          if (!data) {
            alert(`Missing data for Week ${w} ${day}`)
            return
          }
          if (!data.title?.trim()) {
            alert(`Please provide or generate a title for Week ${w} ${day}`)
            return
          }
        }
      }
    ...
    try {
      ...
      // Step 2: Build final scheduleData
      const scheduleData = {
        type: scheduleType,
        weeks: numWeeks,
        time: publishTime,
            ...
            Object.fromEntries(
              Object.entries(days).map(([day, data]: [string, any]) => {
                ...
                return [day, {
                  ...
                  imageUrl: imageUploads[imageKey] || null,
      ...
      const { data: result } = await api.post("/api/workflows", {
        siteId: selectedSite,
        name: workflowName || `${scheduleType} workflow - ${new Date().toLocaleDateString()}`,
        scheduleData,
        tone,
        wordCount,
        language,
  ...
  const selectedDescriptionData = selectedDescriptionDay
    ...
    : null
  const selectedDescriptionKey = selectedDescriptionDay
    ? `${activeWeek}-${selectedDescriptionDay}`
    : null
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="space-y-1.5">
            <Label className="text-xs uppercase tracking-wider text-gray-500 font-bold">
              {contentMode === "auto" ? "Workflow Theme / Name" : "Workflow Name"}
            </Label>
            <div className="flex gap-2">
              <Input 
                placeholder={contentMode === "auto" ? "e.g. 'Coffee Brewing Mastery'" : "e.g. 'My Weekly Updates'"} 
                value={workflowName}
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-top-1">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    onClick={() => {
                      if (contentMode === "auto") {
                        // Clear AI data when switching to manual
                        const clearedPlan = { ...plan }
                        Object.keys(clearedPlan).forEach(week => {
                          Object.keys(clearedPlan[week]).forEach(day => {
                            clearedPlan[week][day].title = ""
                            clearedPlan[week][day].keywords = ""
                          })
                        })
                        setPlan(clearedPlan)
                      }
                                ...
                                <div className="flex items-center gap-2">
                                  <button
                                    onClick={async () => {
                                      ...
                                      await generateDescriptionForDay(activeWeek, day, dayData)
                                    }}
                                    disabled={isDescGenerating[descKey]}
                                  ...
                                  >
                                    {isDescGenerating[descKey] ? <Loader2 className="w-3 h-3 mr-1 animate-spin" /> : <Brain className="w-3 h-3 mr-1" />}
                      ...
                      <div className="min-w-0 self-stretch rounded-lg border border-purple-100 bg-purple-50/30 p-3">
                        ...
                        <div>
                          {selectedDescriptionDay && selectedDescriptionData ? (
                            <>
                              {selectedDescriptionKey && isDescGenerating[selectedDescriptionKey] && (
